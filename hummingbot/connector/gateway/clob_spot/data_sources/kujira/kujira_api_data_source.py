...
from hummingbot.connector.gateway.clob_spot.data_sources.kujira.kujira_helpers import OrderHashManager
...
class KujiraAPIDataSource(CLOBAPIDataSourceBase):
    """An interface class to the Kujira blockchain.
    ...
    async def batch_order_create(self, orders_to_create: List[GatewayInFlightOrder]) -> List[PlaceOrderResult]:
        ...
        async with self._order_placement_lock:
            ...
            if transaction_hash is None:
                ...
                exception = RuntimeError("The creation transaction has failed on the Kujira chain.")
    ...
    async def batch_order_cancel(self, orders_to_cancel: List[InFlightOrder]) -> List[CancelOrderResult]:
        ...
        if transaction_hash is None:
            await self._update_account_address_and_create_order_hash_manager()
            self.logger().error("The batch order update transaction failed.")
            exception = RuntimeError("The cancelation transaction has failed on the Kujira chain.")
    ...
    def _update_local_balances(self, balances: Dict[str, Dict[str, Decimal]]):
        ...
        # details. This is specifically for Kujira during the processing of balance streams, where the messages does not
    ...
    def _parse_trade_event(self, trade: StreamTradesResponse):
        """Kujira fires two trade updates per transaction.
    ...
    async def _listen_to_subaccount_balances_stream(self):
        while True:
            # Uses KujiraAccountsRPC since it provides both total_balance and available_balance in a single stream.
